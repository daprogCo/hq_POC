...
TIMESTAMP_ERR_MSG = "Erreur lors de la récupération de l'horodatage depuis l'API d'Hydro-Québec"
PANNES_ERR_MSG = "Erreur lors de la récupération des données de pannes depuis l'API d'Hydro-Québec"
...
def main():
    timestamp = fetch_data(HQ_API_URL_TIMESTAMP, TIMESTAMP_ERR_MSG)
    if timestamp:
        url = set_url(HQ_API_URL_PANNES, timestamp)
        pannes_data = fetch_data(url, PANNES_ERR_MSG)
        if pannes_data:
            return pannes_data        
